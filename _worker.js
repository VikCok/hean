 ...
 *
 * 职责：
 *  1. 代理 /qmkproxy/* 请求到 api.qmkjcm.cn（解决该站无 CORS 头的问题，
 *     使光遇每日任务 / 一言 / 公告 / 魔法 / 代币 / 文字攻略等接口
 *     可以在浏览器端直接 fetch）。
 *  2. 其余请求一律回退到静态资源（页面 HTML / CSS / JS / 图片等）。
 */
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // ---- 代理路由：/qmkproxy/* -> https://api.qmkjcm.cn/* ----
    if (url.pathname.startsWith("/qmkproxy/")) {
      // 去掉 /qmkproxy 前缀，拼回目标域名，保留原查询参数（如 ?refresh=1）
